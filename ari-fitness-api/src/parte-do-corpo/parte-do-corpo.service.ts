...
const tableName = 'parte_do_corpo';
...
export class ParteDoCorpoService {
   ...
   * selected.
   */
  findAll(filter: Partial<ParteDoCorpo> | ParteDoCorpo) {
   ...
   * @param {ParteDoCorpo} body - The `body` parameter in the `create` function likely represents the data or
   * object of type `ParteDoCorpo` that you want to insert into a database table. It contains the information
   ...
   * data.
   * @param body - The `body` parameter in the `update` function is a partial object of type `ParteDoCorpo`.
   ...
   */
  update(body: Partial<ParteDoCorpo>) {
    return this.database.supabase
      .from(tableName)
      .update(body)
      .eq('id', body.id);
